  ...
  return (
    <section className="relative flex h-screen w-full flex-col items-center justify-center overflow-hidden bg-gradient-to-br from-blue-950 to-purple-950 px-4 text-white">
      ...
      <div className="absolute inset-0 opacity-30">
        <BackgroundCircle size={160} initialPosition={{ x: 10, y: 20 }} duration={8} delay={0} opacity={0.15} />
        <BackgroundCircle size={200} initialPosition={{ x: 75, y: 30 }} duration={12} delay={1} opacity={0.1} />
        <BackgroundCircle size={120} initialPosition={{ x: 60, y: 70 }} duration={10} delay={2} opacity={0.12} />
        <BackgroundCircle size={180} initialPosition={{ x: 25, y: 80 }} duration={9} delay={3} opacity={0.08} />
      </div>
      ...
      {/* Subtle glow effects */}
      <div className="absolute left-1/4 top-1/4 h-64 w-64 -translate-x-1/2 -translate-y-1/2 rounded-full bg-blue-600/5 blur-[100px]" />
      <div className="absolute bottom-1/3 right-1/4 h-72 w-72 -translate-x-1/2 -translate-y-1/2 rounded-full bg-purple-600/5 blur-[100px]" />
      ...
      <div className="relative z-10 max-w-4xl space-y-16 text-center">
        ...
        >
          ...
          <span className="relative">
            <span className="relative z-10 ml-2 font-medium">Artificial Intelligence</span>
            <motion.span 
              className="absolute bottom-0 left-0 z-0 h-[6px] w-full bg-purple-500/30" 
              initial={{ width: 0 }}
              animate={{ width: "100%" }}
              transition={{ duration: 0.8, delay: staggerDelay + 0.5, ease: "easeOut" }}
            />
          </span>
        </motion.h2>
        ...
        >
          <motion.div
            ...
            whileTap={{ scale: 0.97 }}
          >
            <Button asChild intent="primary" className="w-full sm:w-auto px-6 py-3 rounded-md">
